...
#pragma mark - 左对齐 移动
-(void)alignmentImageLeftYD
{
    CGFloat LeftImge = 0;
    if (_LeftImgefl_padding==0) {
        LeftImge = 12.5;
    }else{
        LeftImge = _LeftImgefl_padding;
    }
    //    获得按钮的图片的frame
    CGRect imageFrame = self.imageView.frame;
    //    设置按钮的图片的x坐标紧跟文本的后面
    imageFrame.origin.x = LeftImge;
    //    获得按钮的文本的frame
    CGRect titleFrame = self.titleLabel.frame;
    //    设置按钮的文本的x坐标为0-－－左对齐
    titleFrame.origin.x = CGRectGetWidth(imageFrame) + LeftImge + _fl_padding;
    //    重写赋值frame
    self.titleLabel.frame = titleFrame;
    self.imageView.frame = imageFrame;
}
-(void)alignmentImageRight
{
    // 计算文本的的宽度
    NSMutableDictionary *dictM = [NSMutableDictionary dictionary];
    dictM[NSFontAttributeName] = self.titleLabel.font;
    ...
    CGRect imageFrame = self.imageView.frame;
    imageFrame.origin.x = self.bounds.size.width - frame.size.width- fl_imageWidth - _fl_padding;
    CGRect titleFrame = self.titleLabel.frame;
    titleFrame.origin.x =self.bounds.size.width - frame.size.width ;
    //    重写赋值frame
    self.titleLabel.frame = titleFrame;
    self.imageView.frame = imageFrame;
}
...
- (void)layoutSubviews{
    ...
    else if (_status == FLAlignmentStatusImageRight){
        [self alignmentImageRight];
    }
    else if (_status == FLAlignmentStatusImageLeftYD){
        [self alignmentImageLeftYD];
    }
