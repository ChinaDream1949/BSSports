...
#import "WCPNumberKeyboard.h"

#define downH  0 //10*[UIScreen mainScreen].scale
...
static CGRect wldk_get_frame()
{
    ...
    float safeB = kScreenSafeAreaInset().bottom;
    return CGRectMake(0, 0, width, height+downH+safeB);
}
...
@interface WCPNumberKeyboard () <UIInputViewAudioFeedback>
{
    NSTimer *deleteTimer;
}
...
@implementation WCPNumberKeyboard
...
- (void)_initUI
{
    self.backgroundColor = [UIColor colorWithRed:235 / 255.f
                                           green:235 / 255.f
                                            blue:235 / 255.f
                                           alpha:1.f];
    ...
    for (int i = 0; i < 14; ++i) {
        ...
        if (i == 10) {
            ...
            UILongPressGestureRecognizer *g = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(deleteItemLongPress:)];
            ...
            item.tag = -2;
            [item setTitle:@"付款" forState:UIControlStateNormal];
        }else if (i==14) {
//            item.tag=-3;
//
//            UIImageView *img=[UIImageView new];
//            img.image=wldk_down_icon();
//            img.frame=CGRectMake(0, 0, CGRectGetWidth(self.bounds), downH);
//            [item addSubview:img];
        ...
        if (item.tag != -2) {
            if (item.tag==-3) {
                [item setBackgroundColor:[UIColor colorWithRed:254 / 255.f
                                                         green:254 / 255.f
                                                          blue:254 / 255.f
                ...
                [item setBackgroundColor:[UIColor colorWithRed:254 / 255.f
                                                         green:254 / 255.f
                                                          blue:254 / 255.f
                                                         alpha:1] forState:UIControlStateNormal];
                [item setBackgroundColor:[UIColor colorWithRed:210 / 255.f
                                                         green:210 / 255.f
                                                          blue:210 / 255.f
                                                         alpha:1] forState:UIControlStateHighlighted];
                [item setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        ...
        [item addTarget:self action:@selector(ButtonTouchDown:) forControlEvents:UIControlEventTouchDown];
...
- (void)layoutSubviews
{
    ...
    float safeB = kScreenSafeAreaInset().bottom;
    CGFloat width = (CGRectGetWidth(self.bounds) - wSep*5)/4.f;
    CGFloat height = (CGRectGetHeight(self.bounds)-safeB- wSep*5)/ 4.f;
    
    CGFloat inputH=downH;
    
    [self viewWithTag:-3].frame=CGRectMake(0, 0, CGRectGetWidth(self.bounds), downH);
    //1-3
    [self viewWithTag:49].frame = CGRectMake(wSep, inputH+wSep, width, height);
    [self viewWithTag:50].frame = CGRectMake(width + wSep*2, inputH+wSep, width, height);
    [self viewWithTag:51].frame = CGRectMake((width + wSep) * 2+wSep, inputH+wSep, width, height);
    //4-6
    [self viewWithTag:52].frame = CGRectMake(wSep, inputH+height + wSep*2, width, height);
    [self viewWithTag:53].frame = CGRectMake(width + wSep*2, inputH+height + wSep*2, width, height);
    [self viewWithTag:54].frame = CGRectMake((width + wSep) * 2+wSep, inputH+height + wSep*2, width, height);
    //7-9
    [self viewWithTag:55].frame = CGRectMake(wSep, inputH+wSep + (height + wSep) * 2, width, height);
    [self viewWithTag:56].frame = CGRectMake(width + wSep*2, inputH+wSep + (height + wSep) * 2, width, height);
    [self viewWithTag:57].frame = CGRectMake((width + wSep) * 2+wSep, inputH+wSep + (height + wSep) * 2, width, height);
    
    if (self.style==0) {
        //48 == 0
        [self viewWithTag:48].frame = CGRectMake(width + wSep*2, inputH+wSep + (height + wSep) * 3, width*2+wSep, height);
        [self viewWithTag:46].frame = CGRectMake(wSep, inputH+wSep + (height + wSep) * 3, width, height);
    }else{
        [self viewWithTag:48].frame = CGRectMake(wSep, inputH+wSep + (height + wSep) * 3, width*3+wSep*2, height);
        [self viewWithTag:46].frame = CGRectMake(wSep, inputH+wSep + (height + wSep) * 3, 0, height);
    }
    
    [self viewWithTag:-1].frame = CGRectMake((width + wSep) * 3+wSep, inputH+wSep + (height + wSep) * 3, 20, height);

    
    [self viewWithTag:127].frame = CGRectMake((width + wSep) * 3+wSep, inputH+wSep, width, height + .5f);
    [self viewWithTag:-2].frame = CGRectMake((width + wSep) * 3+wSep, inputH+wSep*2 + height, width, height * 3 + wSep*2);
...
-(void)ButtonTouchDown:(WLDecimalKeyboardItem *)btn{
